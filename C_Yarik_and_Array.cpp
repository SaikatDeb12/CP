// Author: Saikat_deb | Created: 2025-06-14 00:32
...
void solve()
{
    ...
    // here individual elements should also be considered
    int fnlmx = inf_n;
    vi a(n);
    itr(i, n)
    {
    ...
    itr(i, n)
    {
        ...
        for (; j < n; j++)
        {
            if ((flip == 1 and a[j] & 1) or (flip == 2 and a[j] % 2 == 0))
            {
                ind.push_back({i, j - 1});
                i = j - 1;
                break;
    ...
    {
        int i = ind[k].first;
        int till = ind[k].second, j = i;
        int mx = inf_n;
        if (ind[k].first == ind[k].second)
        {
            fnlmx = max(fnlmx, mx);
            continue;
        }
